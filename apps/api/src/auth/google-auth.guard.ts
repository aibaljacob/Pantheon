...
import { GOOGLE_OAUTH_STATE_COOKIE, GOOGLE_OAUTH_STATE_TTL_MS } from './google-auth.constants';
...
export class GoogleAuthGuard extends AuthGuard('google') {
  canActivate(context: ExecutionContext) {
    ...
    const isInitialRedirect = request.originalUrl.includes('/auth/google') && !request.originalUrl.includes('/callback');
  ...
  private getFailureRedirect(request: GoogleAuthRequest, err?: unknown, info?: unknown): string {
  ...
  private resolveErrorCode(request: GoogleAuthRequest, err?: unknown, info?: unknown): string {
    const errorMessage = err instanceof Error ? err.message : '';
    const infoMessage = typeof info === 'string' ? info : info && typeof info === 'object' && 'message' in info ? String((info as { message?: unknown }).message ?? '') : '';
    ...
    if (combinedMessage.includes('access_denied') || combinedMessage.includes('cancel')) {
      return 'google_auth_cancelled';
    }

    return request.originalUrl.includes('/callback') ? 'google_auth_failed' : 'google_auth_unavailable';
  }
}